    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                ...
                <form action="{{ route('admin.loadingbay.finish') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="card-body pt-0">
                        <div class="row">                           
                            <div class="col-md-12">
                                ...
                                <input type="file" class="form-controll" id="environment" name="finishImage"
                                    accept="image/*">
                            </div>
                            <div class="col-md-12 mt-2">
                                @if ($loadingbay->finish_image_url)
                                    {{-- <span class="fw-bold fs-6">Truck Image</span> --}}
                                    <img src="{{ asset( '/outros/'.$loadingbay->finish_image_url ) }}" alt="{{ $loadingbay->truck_details }}" class="img-thumbnail">
                                @endif
                            </div>
                            <div class="col-md-12 mt-2">
                                @can('loadingbay_finish')
                                    @if ($loadingbay->status == 'started_'.$loadingbay->type)
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="hidden" name="id" value="{{ $loadingbay->id }}">
                                        <input type="submit" class="btn btn-dark text-capitalize" value="{{ trans('global.finish') .' '. $loadingbay->type }}">
                                    @endif
                                @endcan
