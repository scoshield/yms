...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.yard.title_singular') }}
        </div>

        <div class="card-body">
            <form action="{{ route('admin.yards.update', [$yard->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                 ...
                 <div class="form-group {{ $errors->has('physical_location') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="physical_location" name="physical_location" class="form-control"
                        value="{{ old('physical_location', isset($yard) ? $yard->physical_location : '') }}">
